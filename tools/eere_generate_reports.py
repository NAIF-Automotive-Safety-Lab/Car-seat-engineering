...
def main() -> int:
    ...
    (ROOT / "EERE_RUNTIME_STATUS.json").write_text(json.dumps({"system": "EERE", "repository_revision": head, "occt": {"status": "VERIFIED", "kernel": result["brep_validation"].get("kernel", "OCP/OpenCascade")}, "step_forensic": result["step_forensic"].get("status", "BLOCKED"), "brep": result["brep_validation"]["status"], "chrono": result["chronoruntime_smoke"], "physical_evidence": "NOT_AVAILABLE", "r4_1_sha256": EXPECTED}, indent=2) + "\n", encoding="utf-8")
